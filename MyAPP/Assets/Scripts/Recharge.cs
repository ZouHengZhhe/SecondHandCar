...
    private string serverIP = "47.110.225.20";
    ...
    public InputField RechargeInput;
    public Button RechargeBtn;

    public delegate void RechargeDel(double money);   //在RestController中被调用
    public RechargeDel RechargeCallback = null;

    private void Start()
    {
        RechargeInput.onValueChanged.AddListener(OnNumInputValueChanged);
        RechargeBtn.onClick.AddListener(BuyObject);
...
        if (zhifu)
        {
            ...
            RechargeCallback(double.Parse(BuyCuont));   //充值成功，调用充值接口通知后台
